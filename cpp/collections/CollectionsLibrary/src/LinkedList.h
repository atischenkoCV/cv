...
public:
	...
	;
	virtual LinkedListIteratorType next() override
	{
		if (hasNext()) {
...
public:
	...
	virtual LinkedListIterator<LinkedListType> iterator() const override
	{
...
protected:
	...
	LinkedListType doGet(unsigned int index) const override
	{
		return doGetNode(index)->value;
	}

	void doAdd(unsigned int index, LinkedListType value) override
	{
	...
	LinkedListType doRemove(unsigned int index) override
	{
	...
	void linkNodes(LinkedNode<LinkedListType>* prev,
			LinkedNode<LinkedListType>* next) const {
		if (prev != nullptr) {
			prev->next = next;
		}
		if (next != nullptr) {
