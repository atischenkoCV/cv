...
#define MAX_LIST_SIZE 32767
  ...
  protected:
    ...
    void
    checkIndex (int lowerBound, int upperBound, int index,
    ...
    const std::string
    getIndexErrorMessage (int lowerBound, int upperBound, int index,
			  const std::string& method) const
    {
      std::string error;
      error.assign ("Index ").append (std::to_string (index)).append (
	  " is out of range for '").append (method).append ("' operation.").append (
	  " Valid range: [").append (std::to_string (lowerBound)).append (", ").append (
	  std::to_string (upperBound)).append ("]");
      return error;
    }
  };
