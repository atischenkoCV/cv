...
public:
	 ...
	 */
	virtual ListType get(unsigned int index) const {
	 ...
	 */
	virtual void add(unsigned int index, ListType value) {
		checkAddIndex(index);
		doAdd(index, value);
	}
	 ...
	 */
	virtual void add(ListType value) {
		checkAddIndex(size());
		doAdd(size(), value);
	}
	/**
	 ...
	 * Index must be greater or equal zero and lower or equal the list's size, in other cases will be thrown exception.
	 */
	virtual ListType remove(unsigned int index) {
...
protected:
	...
	void checkAddIndex(unsigned int index) const {
	...
	void checkIndex(unsigned int upperBound, unsigned int index,
	...
	const std::string getIndexErrorMessage(int upperBound, unsigned int index,
			const std::string& method) const {
		std::string error;
